...
5. 이진화 (OTSU 임계값)
6. 크기 조정 (OCR 모델 입력 크기에 맞춤: 736x736)
...
- **JSON**: 구조화된 데이터 형식
- **신뢰도**: OCR 신뢰도 및 계산 성공 여부 표시

## 6. 구현 순서

1. ✅ 이미지 전처리 함수 작성 (CLAHE, 이진화 추가)
2. ✅ OCR 함수 작성 (기존 코드 활용, 여러 줄 처리 추가)
3. ✅ 수식 정규화 함수 작성 (고급 함수 처리 추가)
4. ✅ 고급 계산기 구현 (지수, 삼각함수, 로그, 제곱근, 절댓값)
5. ✅ 메인 함수 통합 (DnnFormulaRecognitionAdvanced)
6. ✅ 형식 변환 추가 (AsciiMath, LaTeX)
7. ✅ 결과 시각화 추가 (탭 기반 다이얼로그)
8. ✅ 예외 처리 추가
9. ✅ 신뢰도 계산 추가
10. ✅ 테스트 및 디버깅
